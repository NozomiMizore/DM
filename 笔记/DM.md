...
- CF树上不保存任何数据点，仅有树的结构信息以及相关的聚类特征信息，因此CF树可以达到压缩数据的效果
- Birch算法为什么适合于处理大数据
  - CF 结构概括了簇的基本信息，并且是高度压缩的，它存储了小于实际数据点的聚类信息。每个新添加的数据其作为个体消失了，将信息融入的集合簇中
  - 增量式的学习方法，不用一次将数据全部加载到内存，可以一边添加数据一边进行学习
#### CF树的构建有参数L：节点中最大的cf数，T：距离半径阈值
- L用来限制节点中的CF个数
- T用来限制CF的最大半径

- T过小时对聚类限制过于苛刻，会使得节点过多，树的高度过大，构造过程开销较大，产生过多的类。
- T过大时会使得类半径过大，类内数据过多，类内数据关联度差，聚类效果差，一些异常点也不容易被检查出来
- L过小时会导致树的高度过大，层数过多，构造过程开销较大，聚类效果不理想。
- L过大时会导致书的高度降低，层数过少，聚类效果不理想。
#### 构建CF树步骤
  1. 从根节点向下寻找和新样本距离最近的叶子节点和叶子节点里最近的CF节点
  2. 新样本加入后，若CF节点对应的超球体半径小于阈值T，则更新路径上所有CF三元组，插入结束。否则转入3(需要创建新的CF用以容纳该样本)
  3. 若当前叶子节点的CF节点个数小于阈值L，则创建一个新CF节点，放入新样本，将新的CF节点放入这个叶节点，更新路径上所有的CF三元组，插入结束。否则转入4
